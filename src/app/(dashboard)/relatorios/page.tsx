...
import { useContas, useFornecedores } from '@/hooks';
...
export default function RelatoriosPage() {
  const { contas, listarContas } = useContas();
  const { fornecedores, listarFornecedores } = useFornecedores();
  ...
  const totalGeral = useMemo(() => {
    return contas.reduce((acc, curr) => acc + Number(curr.valor), 0);
  }, [contas]);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `relatorio-contas-${dataInicio}-${dataFim}.csv`;
    a.click();
  };
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Visualize e exporte dados das contas
            </p>
          </div>
          <Button variant="outline" onClick={exportCSV} disabled={contas.length === 0}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-primary/10 rounded-lg">
                ...
                <span className="text-2xl font-bold">{contas.length}</span>
              </div>
