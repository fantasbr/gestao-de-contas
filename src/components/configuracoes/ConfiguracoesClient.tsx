...
export function ConfiguracoesClient({ initialWebhooks, hasToken }: ConfiguracoesClientProps) {
  ...
  const handleOpenDialog = (webhook?: Webhook) => {
    if (webhook) {
      setEditando(webhook);
      setFormData({
        nome_evento: webhook.nome_evento,
                ...
                <Dialog open={showDialog} onOpenChange={setShowDialog}>
                  ...
                  <DialogContent className="max-w-md">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        {editando ? 'Edite as informações do webhook.' : 'Selecione o tipo de webhook que deseja configurar.'}
                      </DialogDescription>
                    </DialogHeader>
                            ...
                            <Select onValueChange={handleWebhookSelecionado}>
                              ...
                              <SelectContent>
                                {WEBHOOKS_PREDEFINIDOS.map((webhook) => (
                                  <SelectItem key={webhook.nome_evento} value={webhook.nome_evento}>
                    ...
                    {webhooks.map((webhook) => (
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground mt-1">{webhook.url_webhook}</p>
                          {webhook.descricao && (
                            <p className="text-xs text-muted-foreground mt-1">{webhook.descricao}</p>
                          )}
                        </div>
                        <div className="flex gap-2 ml-4">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleExcluir(webhook.id)}>
